...
<button
	class="{dark
		...
		: 'bg-blue-100 focus:ring-blue-400 ring-offset-white'} relative inline-flex flex-shrink-0 h-5 w-9 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 m-4"
	class:hidden
	type="button"
	aria-checked={dark}
	on:click={toggle}
>
	...
	>
		...
		>
			<!-- sun icon -->
			<svg class="h-3 w-3 text-blue-400" viewBox="0 0 20 20" fill="currentColor">
				<path
					fill-rule="evenodd"
...
<button
	class="{dark
		...
		: 'focus:ring-blue-400 ring-offset-white'} border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 "
	class:hidden
	on:click={toggle}
>
	<!-- moon icon --> 
	<img src="/arblogo.png" class="h-6 w-6 animate-pulse object-cover text-gray-400 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 outline-none ring-2 ring-offset-2 ring-offset-black/20">
		
	
</button>
<span class="text-black/50 dark:text-white/50 font-semibold text-sm ml-2">Arbitrum</span>
